<template>
    <div>
        <v-app-bar color="greenD">
            ...
            <v-menu offset-y>
                ...
                <v-list>
                    ...
                    <v-list-item v-for="(item, index) in listausercon" :key="index">
                        <v-list-item-title>
                            <v-icon class="greenD--text">mdi-account</v-icon>{{
                            item.name }} <v-btn class="greenD white--text" @click="enviarNotificacion(item)">
            ...
            <v-dialog v-model="sendnot" max-width="500">
                <v-card>
            ...
            <v-dialog v-model="hisview" fullscreen>
                <v-card>
                    ...
                    <v-card-text>
                        <v-data-table :headers="tithis" :items="todochat">
                        </v-data-table>
                    </v-card-text>
                </v-card>
            </v-dialog>
        ...
        <v-container class="fill-height "
            style="min-height:350px; max-height:400px; height: 100%; overflow:auto; padding-bottom:10px;"
            ref="conversations">
...
export default {
    ...
    mounted() {
        ...
        this.socket.on('message', (data) => {
            ...
            this.listausercon = []
            this.listausercon = data.usuarios
        });
