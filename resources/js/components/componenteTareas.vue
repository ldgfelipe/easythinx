    ...
    <div>
        <v-card min-height="550px">
            <v-card-title class="greenD white--text">
                ...
                <v-btn v-if="proyectoSelect.adminproy" class="white greenD--text" @click="abreAddTarea()">
            ...
            <v-card-text>
                ...
                <v-row>
                    <v-col cols="8" md="8">
                    <v-text-field outlined v-model="searchgral" label="Buscar tarea" ></v-text-field>
                    </v-col>
                    <v-col cols="4" md="4" >
                        <v-select v-model="filstatus" label="Status"  filled :items="tipoTareas"></v-select>
                    </v-col>
                </v-row>
          <v-btn text @click="filtros===false ? filtros=true : filtros=false">Filtrar</v-btn>

        

                <v-data-table :headers="titulotareas" :search="searchgral" :items="tareas">

                    <template v-slot:item.asignado="{ item }">
                      
                        <v-chip
                            v-if="JSON.parse(item.asignado).filter(a=>a===sesion.email)[0]===sesion.email ? true: false"
                            color="greenD">
                    ...
                    <template v-slot:item.caducidad="{ item }">
                        <v-chip v-if="item.caducidad" :color="validacaducidad(item.caducidad) ?  'greenD' : 'Red'"  :style="validacaducidad(item.caducidad) ?  'font-size:9px; color:black;' : 'font-size:9px; color:white;'" >{{item.caducidad }}</v-chip>
                    </template>
        ...
        <v-dialog v-model="addtarea" max-width="700" persistent>
            <v-card>
                ...
                <v-card-text>
                    <br />
                    <v-text-field v-if="proyectoSelect.adminproy" label="Nombre de la Tarea" outlined
                        v-model="addservice.titulo"></v-text-field>
                    <v-banner v-if="!proyectoSelect.adminproy"><b>{{addservice.titulo}}</b></v-banner>

                    <v-textarea v-if="proyectoSelect.adminproy" label="Sinopsis" outlined v-model="addservice.desc">
                    </v-textarea>

                    <v-text-field type="date" v-model="addservice.caducidad" label="Fecha de Caducidad" ></v-text-field>

                    <v-banner v-if="!proyectoSelect.adminproy">{{addservice.desc}}</v-banner>
                    <br /><br />

                    <v-row v-if="addservice.action==='editar'">
                        <v-col cols="12" md="10">
                            <v-text-field v-model="AddObs" outlined label="Agregar observaciones a tarea">
                            </v-text-field>
                            <v-chip class="red white--text" v-if="mensajeobstext">{{mensajeobstext}}</v-chip>
                        </v-col>
                        <v-col cols="12" md="2">
                            <v-btn class="greenD" v-if="AddObs.length>0" @click="addtextobs()">
                                <v-icon class="white--text">mdi-content-save-plus</v-icon>
                            </v-btn>
                        </v-col>
                        <v-col cols="12" md="12">
                            <v-spacer></v-spacer>
                            ...
                            <v-dialog v-model="notasfull" fullscreen persistent>
                                <v-card>
                                    <v-card-title class="greenD white--text">
                                        ...
                                        <v-btn class="white " @click="notasfull=false">
                                            <v-icon class="greenD--text">mdi-close</v-icon>
                                        </v-btn>
                                    </v-card-title>
                                    <v-card-text>
                                        <v-text-field v-model="search"
                                            label="Ingrese a palabra o frase que desea buscar"></v-text-field>
                                        <v-data-table :headers="notastitulo" :items="addservice.obs" :search="search">
                                            <template v-slot:item.action="{ item }">
                                                <v-btn v-if="proyectoSelect.adminproy" @click="eliminaObservacion(item)"
                            ...
                            <v-divider></v-divider>
                                ...
                                <v-data-table :headers="notastitulomini" :items="addservice.obs" :search="search"
                                    hide-default-footer>
                                    <template v-slot:item.action="{ item }">
                                        <v-btn v-if="proyectoSelect.adminproy" @click="eliminaObservacion(item)"
                                            class="greenD white--text">
                    ...
                    <v-select v-if="proyectoSelect.adminproy" v-model="addservice.asignado" chips
                        label="Tarea asignada a:" multiple outlined :items="userproy"></v-select>
                    ...
                    <v-progress-circular v-if="stsave" :width="4" color="green" indeterminate></v-progress-circular>
                    <v-chip color="greenD" v-if="stsave" class="white--text">Guardando Cambios espere un momento</v-chip>


                    <v-btn v-if="!stsave" class="greenD white--text" @click="GuardarTarea()">
                        <v-icon>mdi-content-save</v-icon>
                    </v-btn>

                    <v-btn v-if="proyectoSelect.adminproy && addservice.action==='editar'"  class="error white--text" @click="eliminatarea(item)">
                            <v-icon>mdi-delete</v-icon>
                        </v-btn>
...
export default {
    ...
    computed: {
        ...mapState(['tareas', 'usuarios', 'userproy', 'proyectoSelect', 'csrf', 'sesion', 'url','socket','moment']),
        titulotareas(){ 
            return [
                {
                    text: 'Titulo',
    ...
    methods: {
        ...
        eliminaObservacion(p) {
            if (confirm('¿Seguro que desea eliminar la tarea seleccionada?')) {
                let arraymodif = this.addservice.obs.slice(this.addservice.obs.indexOf(p))
        ...
        GuardarTarea() {
            ...
            if(!this.addservice.titulo){
                error.push('Debe ingresar el <b>Titulo o Nombre de Tarea</b> para continuar')
                mensajes+="Debe ingresar el <b>Titulo o Nombre de Tarea</b> para continuar<br />"
            }
